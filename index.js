...
function extractTextInParentheses(str) {
  ...
  const texts = Array.from(matches, (match) => match[1])
...
function countEmails(str) {
  ...
  const countEmails = matches ? matches.length : 0
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const changeWord = str.replace(regexp, newWord)
...
function checkRegexMethods(regex) {
  ...
  const reDotall = regex.dotAll ? 'dotAll' : null
  // Перевіряємо, чи використовується метод `multiline`.
  const reMultiline = regex.multiline ? 'multiline' : null
  // Перевіряємо, чи використовується метод `sticky`.
  const reSticky = regex.sticky ? 'sticky' : null
  // Повертаємо масив використаних методів.
  arr.push(reDotall, reMultiline, reSticky)
  return arr
}
...
function findWord(str, word) {
  ...
  const regSearch = str.search(regexp)
  return regSearch
}
